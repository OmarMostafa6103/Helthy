...
const ProfilePage = () => {
  ...
  const handleEditProfile = async () => {
    ...
    if (editedData.current_password)
      formData.append("current_password", editedData.current_password);
    if (editedData.password) formData.append("password", editedData.password);
    if (editedData.password_confirmation)
      formData.append(
        "password_confirmation",
        editedData.password_confirmation
      );
    ...
    if (window.confirm("Are you sure you want to delete your email?")) {
      try {
        const response = await fetch(
          `${backendUrl}/api/profile?_method=DELETE`,
          {
            method: "POST",
            headers: {
              Authorization: `Bearer ${token}`,
      ...
      <div className="w-full max-w-3xl bg-white dark:bg-[#1a2338] rounded-2xl shadow-2xl p-8">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-500 dark:text-gray-300 mt-2">
            قم بإدارة معلوماتك الشخصية وتحديثها
          </p>
        </div>
        ...
        {editMode ? (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-sm">
            {[
              ...
              <div key={field}>
                <label className="text-gray-700 dark:text-gray-200 font-semibold block mb-1 capitalize">
                  {field.replace("_", " ")}:
                </label>
                <input
                  type={field.includes("password") ? "password" : "text"}
            ...
            <div>
              <label className="text-gray-700 dark:text-gray-200 font-semibold block mb-1">
                Profile Image:
              </label>
              <input
                type="file"
          ...
          <div className="space-y-6 divide-y divide-gray-200 dark:divide-gray-700">
            ...
            <div className="flex items-center gap-4 pt-4">
              ...
              <p className="text-gray-800 dark:text-gray-100">
                {profileData?.address || "لا يوجد عنوان"}
              </p>
            </div>
          </div>
        )}
