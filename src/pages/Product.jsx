...
const Product = () => {
  ...
  const fetchProductData = useCallback(async () => {
    ...
    try {
      ...
      setSelectedImage(
        formattedProduct.image[0] || "/path/to/placeholder-image.jpg"
      );
    } catch (error) {
  ...
  const handleAddToCart = () => {
    if (productData) {
      if (quantityInCart + 1 > productData.quantity) {
        toast.error(
          `لا يمكن تحديد أكثر من ${productData.quantity} وحدة في السلة!`,
          {
            style: { background: "red", color: "white" },
          }
        );
  ...
  const scrollToProductSection = () => {
    if (productSectionRef.current) {
      const topPosition =
        productSectionRef.current.getBoundingClientRect().top + window.scrollY;
      window.scrollTo({
        top: topPosition,
        behavior: "smooth",
      });
    ...
    return (
      <div className="text-center text-gray-500 pt-10">
        <p className="text-lg mb-4">
          المنتج غير موجود أو حدث خطأ في تحميل البيانات.
        </p>
        <button
          onClick={fetchProductData}
          className="mt-4 px-4 py-2 bg-green-600 text-white rounded hover:bg-green-700"
